...
**Q2: 如何修改采样率？找不到编辑框。**
**A:** 主GUI界面采样率显示为只读文本。请使用`重采样`按钮来修改采样率，这会触发安全的重采样流程。
...
**A:** GUI 的“说明文档”按钮会在运行时尝试打开同目录下的 `README.md`（若不存在任何 `.md` 文件会弹窗提示），请把 README 放在与 `.m` 相同的文件夹。且流程图的图片文件夹assets也需要放在和`README.md`相同的文件夹。
